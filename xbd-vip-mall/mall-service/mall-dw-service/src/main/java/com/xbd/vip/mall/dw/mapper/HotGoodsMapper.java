...
public interface HotGoodsMapper extends BaseMapper<HotGoods> {
     ...
     * 查询过去几小时前N条记录
     */
    @Select("SELECT ip,uri,__time as accesstime FROM mslogs WHERE __time>= TIMESTAMP '${time}' LIMIT #{size}")
    List<HotGoods> search(@Param("size")Integer size,@Param("time")String time);

    /***
     * 分页查询+排序
     */
    @Select("SELECT ip,uri,__time as accesstime FROM mslogs ORDER BY ${sort} ${sortType} LIMIT #{size} offset #{offset}")
     ...
     * 分页查询
     */
    @Select("SELECT ip,uri,__time as accesstime FROM mslogs LIMIT #{size} offset #{offset}")
